			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body" itemscope itemtype="http://schema.org/UserComments">
				...
				<div class="comment-text">
					...
					<?php printf( '<div class="comment-author" itemscope itemprop="creator" itemtype="http://schema.org/Person">%1$s %2$s</div>',
						...
						( $comment->user_id === $post->post_author ) ? '<span class="comment-author__post-author-label">' . esc_html__( 'Post author', 'emaus' ) . '</span>' : ''
					); ?>
						...
						<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>" class="comment-date">
							<time datetime="<?php comment_time( 'c' ); ?>" itemprop="commentTime">
								<?php
									...
									printf( esc_html__( '%1$s at %2$s', 'emaus' ), get_comment_date( '', $comment ), get_comment_time() );
								?>
							</time>
						</a>
						<?php edit_comment_link( esc_html__( 'Edit', 'emaus' ), '<span class="comment-edit-link">', '</span>' ); ?>
					</div><!-- .comment-metadata -->

					<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'emaus' ); ?></p>
					<?php endif; ?>
